...
void Sequence::copyStringIntoVector(vector<char>* _vectorTo, string* _stringFrom)
{
    ...
    for(int i = 0; i < _stringFrom->size(); i++)
    {
    ...
    {
        std::cout << "Error: In function copyStringIntoVector. Strings different length!\n";
        exit(1);
    }
}
...
void Sequence::checkIntegrity()
{
    ...
    {
        std::cout << "Error: _sequence is not same size as _encodedSequence\n";
        exit(1);
    }
}
