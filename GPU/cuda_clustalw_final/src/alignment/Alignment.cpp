...
void Alignment::resetProfile1()
{
    ...
    {
        sl = 0;
        for (j = 0; j < gapPenaltyMask1.size(); ++j) 
        {
    ...
    {
        sl = 0;
        for (j = 0; j < secStructMask1.size(); ++j) 
        {
...
void Alignment::resetProfile2()
{
    ...
    {
        sl = 0;
        for (j = 0; j < gapPenaltyMask2.size(); ++j) 
        {
    ...
    {
        sl = 0;
        for (j = 0; j < secStructMask2.size(); ++j) 
        {
...
bool Alignment::addOutputIndex(vector<int>* outputIndexToAdd)
{
    ...
    if(outputIndexToAdd->size() == numSeqs)
    {
...
{
    // Check if the size is correct
    if((outputIndexToAppend->size() + outputIndex.size()) == numSeqs)
    {
        ...
        {
            clearAlignment();
            cout << "There is a problem with adding the sequences\n";
            return false;
        }
    }
    else
                                           ...
                                           bool explicitPasteToProfile2)
{
    ...
    int initialProf1NumSeqs = prof1NumSeqs;        
...
bool Alignment::testUniqueNames(vector<Sequence>* seqVector)
{
...
void Alignment::addSequencesToVector(vector<Sequence>* seqVector)
{
    ...
    if(!((seqArray.size() == numSeqs + 1) && (names.size() == numSeqs + 1)
          && (titles.size() == numSeqs + 1) && (sequenceIds.size() == numSeqs + 1)))
    {
        cout << "There has been an error adding the sequences to Alignment.\n"
...
void Alignment::printSequences()
{
    for(int i = 0; i < seqArray.size(); i++)
    {
        for(int j = 0; j < seqArray[i].size(); j++)
        {
                cout << seqArray[i][j] << "\t";
        }
        cout << std::endl;
...
void Alignment::calculateMaxLengths()
{
    ...
    if(seqArray.size() > 0)
    {
        ...
        while(seqArrayIter != seqArray.end())
        {
            ...
            if((*seqArrayIter).size() - 1 >= lengthLongestSequence)
            {
 ...
 */
bool Alignment::checkAllNamesDifferent()
{
    bool different = true;
...
{
    if(index > 0 && index < secStructMask1.size())
    {
...
{
    if(index > 0 && index < secStructMask2.size())
    {
...
{
    if(index > 0 && index < gapPenaltyMask1.size())
    {
...
{
    if(index > 0 && index < gapPenaltyMask2.size())
    {
...
{
    if(index >= 0 && index < outputIndex.size())
    {
...
{
    if(index >= 0 && index < seqWeight.size())
    {
...
void Alignment::removeGapsFromSelectedSeqs(vector<int>* selected)
{
    ...
    for(int i = 1; i <= getNumSeqs() && i < size; i++)
    {
        if((*selected)[i] == 1)
        {
            ...
            if(seqArray[i].size() > numExtraElements)
            {
                for(int k = 0; k < numExtraElements; k++)
                {
                    seqArray[i].pop_back();
...
void Alignment::removeGapOnlyColsFromSelectedSeqs(vector<int>* selected)
{
    ...
    for(int i = 1; i <= getSeqLength(firstSeqSelected);)
    {
        ...
        if(numGaps == numColumns)
        {
            ...
            for(int j = firstSeqSelected; j < sizeSelected && (*selected)[j] == 1; j++)
            {
                for(k = i + 1; k <= getSeqLength(j) + 1 && seqArray[j].size() > k; k++)
                {
...
bool Alignment::removeAllOutsideRange(int beginPos, int endPos)
{
    ...
    if(!ok)
    {
        cout << "There was a problem removing a portion of the array\n";
        return false;
    }
...
void Alignment::printSeqArray(SeqArray* arrayToPrint)
{
...
bool Alignment::reloadAlignment()
{
    ...
    if(outputIndex.size() != getNumSeqs())
    {
        outputIndex.clear();
        return false;
    }
    ...
    { 
        if(i < outputOrderSeqArray.size() && i - 1 < outputIndex.size() &&
...
{
    if(index >= 1 && index < seqArray.size())
    {
        seqArray[index] = *seq;
    }
}
