...
#include "Help.h"
#include <ctime>
#include "cuda/cudaFullPairwiseAlign.h"
#include "cuda/cudaMultiSeqAlign.h"
...
{   
    ...
    time_t ltime_start_all, ltime_end_all;
    struct timeb tstruct_start_all, tstruct_end_all;
    ftime(&(tstruct_start_all));
    time(&(ltime_start_all));
    ...
    struct timeb tstruct_start, tstruct_end;

    ftime(&(tstruct_start));
    time(&(ltime_start));

    float dist_time = 0.f;
    cudaFullPairwiseAlign( &alignmentObj, &distMat, 0, static_cast<int>(_numSeqs), 0, static_cast<int> (_numSeqs));

    ftime(&(tstruct_end));  
    time(&(ltime_end));
    dist_time = (ltime_end-ltime_start) + (tstruct_end.millitm-tstruct_start.millitm)*0.001;          

    //dist_time = clock() - dist_time;
/*
...
*/   
    ...
    float guide_tree_time = 0.f;

    ftime(&(tstruct_end));  
    time(&(ltime_end));
    guide_tree_time = (ltime_end-ltime_start) + (tstruct_end.millitm-tstruct_start.millitm)*0.001;          
  ...
  /*  
    double msa_time = clock();
    count = cudaMultiSeqAlign(&alignmentObj, &distMat, &seqWeight, progSteps.get(), 0);
    msa_time = clock() - msa_time;
  */  
    ...
    ftime(&(tstruct_end));  
    time(&(ltime_end));
    float msa_time = (ltime_end-ltime_start) + (tstruct_end.millitm-tstruct_start.millitm)*0.001;          
    ...
    ftime(&(tstruct_end_all));  
    time(&(ltime_end_all));
    float FullMSATime = (ltime_end_all-ltime_start_all) + (tstruct_end_all.millitm-tstruct_start_all.millitm)*0.001;          


    cout << "pairwise time      : " << dist_time << endl;
    //cout << "guide tree time    : " << guide_tree_time << endl;
    //cout << "MSA time           : " << msa_time << endl;
    cout << "Full MSA total time: " << FullMSATime << endl;
    return;
}
...
void Clustal::sequencesAlignToProfile(string* phylipName)
{
   ...
   //
    ...
    if (userParameters->getNewTreeFile()) // WHAT IS THIS?????????????
    {
        return;
    }
 ...
 */
int Clustal::sequenceInput(bool append)
{
    int code;
    ...
    code = readSeqFile.seqInput(&alignmentObj, append);
    return code;
}
...
int Clustal::profile1Input(string profile1Name)
{
    ...
    if(!userParameters->getInteractive())
    {
        if(code != OK)
        {
            cerr << "ERROR: There are no sequences in profile1 file.\n";            
            exit(2);
        }
    }
...
{    
    ...
    if(!userParameters->getInteractive())
    {
        if(code != OK)
        {
            cerr << "ERROR: There are no sequences in profile2 file.\n";
            exit(2);
        }
    }      
    return code;
...
int Clustal::commandLineReadSeq(int firstSeq)
{
    ...
    code = readInputFile.readSeqs(&alignmentObj, firstSeq);    
    
    if(code != OK) 
    {
        if(code == CANNOTOPENFILE)
        {
            cout << "Cannot open input file. No alignment!\n";
        }
        else if(code == NOSEQUENCESINFILE)
        {
            cout << "No sequences in file. No alignment!\n";
        }
        else if(code == ALLNAMESNOTDIFFERENT)
        {
            cout << "ERROR: Multiple sequences found with same name!\n";            
        }
        else if(code == EMPTYSEQUENCE)
        {
            cout << "ERROR: Empty sequences found!\n";            
        }
        else if(code == SEQUENCETOOBIG)
        {
            cout << "ERROR: sequence(s) too big!\n";            
        }
        else if(code == BADFORMAT)
        {
            cout << "ERROR: Sequences are badly formatted!\n";            
        }
        else
        {
