                                    ...
                                    int& prfLength2, SeqArray* seqArray)
{
    ...
    int gap = userParameters->getGapPos1();    
        // Copy the SeqArray!
    for (int j = 0; j < numSeqsProf1 + numSeqsProf2 && j < seqArray->size(); j++)
    {
        profileSeqs[j + 1].clear();
        profileSeqs[j + 1].resize(prfLength1 + 1);
        for (int i = 0; i < prfLength1 && i < (*seqArray)[j].size(); i++)
        {
    ...
    for(int n = 1; n <= iterations; n++)
    {
        ...
        for(int i = 1; i <= nSeqs; i++)
        {
            ...
            if(nSeqs - 1 >= 2) 
            {
                ...
                string treeName;
                treeName = "tempIterTree.dnd";
                p2TreeName = string(treeName);
