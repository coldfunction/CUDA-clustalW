                                         ...
                                         vector<int>* group, int* aligned)
{
    ...
    for (i = 0; i < numSeq; i++)
    {
        if ((*group)[i + 1] == 1)
        {
            ...
            seqArray[nseqs1] = vector<int>(alnPtr->getSequence(i + 1)->begin() + 1, alnPtr->getSequence(i + 1)->end());
    ...
    int alnLengthTemp = prfLength1 + prfLength2 + 2;
//    alnPtr->setMaxAlnLength(prfLength1 + prfLength2 + 2);
    ...
    gaps.resize(alnLengthTemp + 1);
    
    bool profile1Pen; 
    if (switchProfiles == false)
    {
        profile1Pen = userParameters->getStructPenalties1() && userParameters->getUseSS1();
        profileWithSub->calcGapCoeff(&seqArray, &gaps, profile1Pen, alnPtr->getGapPenaltyMask1(), gapcoef1, lencoef1);
    }
    else
    {
    ...
    int _maxAlnLength = alnLengthTemp;
    ...
    if(userParameters->getDoRemoveFirstIteration() == TREE)
    {
        ...
        iterateObj.iterationOnTreeNode(numSeqsProf1, numSeqsProf2, prfLength1, prfLength2, &seqArray);           
    }
...
int MyersMillerProfileAlign::progDiff(int A, int B, int M, int N, int go1, int go2)
{
    ...
    int t, tl, g, h;
    {
        ...
        if (M <= 1)
        {
            ...
            {
                midh =  - gapPenalty2(A + 1, B, 1) - gapPenalty1(A + 1, B + 1, N);
            }
            midj = 0;
            for (j = 1; j <= N; j++)
            {
...
void MyersMillerProfileAlign::addGGaps(Alignment* alnPtr, SeqArray* seqArray)
{
    ...
    for (j = 0; j < nseqs1; j++)
    {
        ...
        for (i = 0; i < alignmentLength; i++)
        {
            if (alnPath1[i] == 2)
            {
                if (ix < ((*seqArray)[j].size() - extraEndElemNum))
                {
                    ta[i] = (*seqArray)[j][ix];
                }
                else
    ...
    {
        cout << "ddddddd1" << endl;
        addGGapsMask(alnPtr->getGapPenaltyMask1(), alignmentLength, &alnPath1, &alnPath2);
    }
    if (userParameters->getStructPenalties1() == SECST)
    {
        cout << "ddddddd2" << endl;
        addGGapsMask(alnPtr->getSecStructMask1(), alignmentLength,  &alnPath1, &alnPath2);
    }

    if (userParameters->getStructPenalties2() != NONE)
    {
        cout << "ddddddd3" << endl;
        addGGapsMask(alnPtr->getGapPenaltyMask2(), alignmentLength, &alnPath2, &alnPath1);
    }
    if (userParameters->getStructPenalties2() == SECST)
    {
        cout << "ddddddd4" << endl;
        addGGapsMask(alnPtr->getSecStructMask2(), alignmentLength, &alnPath2, &alnPath1);
    }
 }
