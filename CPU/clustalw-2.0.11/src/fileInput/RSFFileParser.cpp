...
vector<Sequence> RSFFileParser::getSeqRange(int firstSeq, int no)
{
 ...
 */
Sequence RSFFileParser::getSeq(int seqNum)
{
    ...
    try
    {
        ...
        if (characterSeq.length() > userParameters->getMaxAllowedSeqLength())
        {
 ...
 * @return 
 */
bool RSFFileParser::keyword(char *line,char *code)
{
    int i;
    char key[MAXLINE];
