...
class FileParser
{
    public:
        ...
        FileParser();
        virtual ~FileParser();
        virtual vector<Sequence> getSeqRange(int firstSeq, int num) = 0; // VIRTUAL
        virtual Sequence getSeq(int seqNum) = 0; // VIRTUAL
        virtual int countSeqs() = 0; // VIRTUAL 
        virtual void getSecStructure(vector<char>& gapPenaltyMask, 
                                     vector<char>& secStructMask,
                                     string& secStructName, int &structPenalties, int length) = 0; //VIRTUAL
