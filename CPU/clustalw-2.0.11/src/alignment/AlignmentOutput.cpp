...
void AlignmentOutput::createAlignmentOutput(Alignment* alignPtr, int firstSeq, int lastSeq)
{
    ...
    {
        cout << "ERROR: Cannot produce alignment output. " 
    ...
    try
    {
        ...
        if (userParameters->getRangeFromToSet()) 
        {
            ...
            {
                cout << "seqrange numbers are not set properly, using default....\n";
                firstRes = 1;
                lastRes = length;
            }
            else
...
void AlignmentOutput::fastaOut(Alignment* alignPtr, outputRegion partToOutput)
{
    ...
    {
        fastaOutFile->close();
        cout << "A bad_alloc exception has occured in the fastaOut function.\n"
             << e.what() << "\n";
        exit(1);
    }
    ...
    {
        fastaOutFile->close();
        cout << "An exception has occured in the fastaOut function.\n"
             << e.what() << "\n";
        exit(1);    
    }
    catch(...)
...
void AlignmentOutput::gcgOut(Alignment* alignPtr, outputRegion partToOutput)
{
    ...
    {
        gcgOutFile->close();
        cout << "A bad_alloc exception has occured in the gcgOut function.\n"
             << e.what() << "\n";
        exit(1);
    }
    catch(VectorOutOfRange e)
    {
        gcgOutFile->close();
        cout << "An exception has occured in the gcgOut function.\n"
             << e.what() << "\n";
        exit(1);    
    }
    catch(...)
...
void AlignmentOutput::phylipOut(Alignment* alignPtr, outputRegion partToOutput)
{
    ...
    try
    {
        ...
        {
            warn = false;
            for(i = 0; i < _seqNames.size() - 1; i++)  
            {
                for(j = i + 1; j < _seqNames.size(); j++) 
                {
    ...
    {
        phylipOutFile->close();
        cout << "A bad_alloc exception has occured in the phylipOut function.\n"
             << e.what() << "\n";
        exit(1);
    }
    catch(VectorOutOfRange e)
    {
        phylipOutFile->close();
        cout << "An exception has occured in the phylipOut function.\n"
             << e.what() << "\n";
        exit(1);    
    }
    catch(...)
...
void AlignmentOutput::nexusOut(Alignment* alignPtr, outputRegion partToOutput)
{
    ...
    {
        nexusOutFile->close();
        cout << "A bad_alloc exception has occured in the nexusOut function.\n"
             << e.what() << "\n";
        exit(1);
    }
    catch(VectorOutOfRange e)
    {
        nexusOutFile->close();
        cout << "An exception has occured in the nexusOut function.\n"
             << e.what() << "\n";
        exit(1);    
    }
    catch(...)
...
void AlignmentOutput::gdeOut(Alignment* alignPtr, outputRegion partToOutput)
{
    ...
    {
        gdeOutFile->close();
        cout << "A bad_alloc exception has occured in the gdeOut function.\n"
             << e.what() << "\n";
        exit(1);
    }
    catch(VectorOutOfRange e)
    {
        gdeOutFile->close();
        cout << "An exception has occured in the gdeOut function.\n"
             << e.what() << "\n";
        exit(1);    
    }
    catch(...)
...
void AlignmentOutput::nbrfOut(Alignment* alignPtr, outputRegion partToOutput)
{
    ...
    {
        nbrfOutFile->close();
        cout << "A bad_alloc exception has occured in the nbrfOut function.\n"
             << e.what() << "\n";
        exit(1);
    }
    catch(VectorOutOfRange e)
    {
        nbrfOutFile->close();
        cout << "An exception has occured in the nbrfOut function.\n"
             << e.what() << "\n";
        exit(1);    
    }
    catch(...)
...
void AlignmentOutput::clustalOut(Alignment* alignPtr, outputRegion partToOutput)
{
    ...
    try
    {
        ...
        for(lv1 = 1; lv1 <= chunks; ++lv1) 
        {
            ...
            if(secStructName1.size() > MAXNAMESTODISPLAY)
            {
            ...
            if (_outStructPenalty == 1 || _outStructPenalty == 2) 
            {
                    ...
                    userParameters->getUseSS1() == true) 
                {
                    ...
                    for(i = pos; i <= ptr; ++i) 
                    {
                        if(_gapPenaltyMask1->size() > i + firstRes - 2) 
                        {
            ...
            //if(secStructName2.size() > MAXNAMESTODISPLAY)
            if(secStructName2.size() > NAMESWIDTH)
            {
            ...
            if (_outStructPenalty == 1 || _outStructPenalty == 2) 
            {
                    ...
                    userParameters->getUseSS2() == true) 
                {
                    ...
                    for(i = pos; i <= ptr; ++i) 
                    {
                        if(_gapPenaltyMask2->size() > i + firstRes - 2)
                        {
            ...
            for(ii = firstSeq; ii <= lastSeq; ++ii) 
            {
                ...
                //if(seqName.size() > MAXNAMESTODISPLAY)
                if(seqName.size() > NAMESWIDTH)
                {
            ...
            for(i = pos; i <= ptr; ++i) 
            {
                ...
                for(j = 1; j <= strongGroup.size(); j++)
                {
                    catident1[j - 1] = 0;
                }
                for(j = 1; j <= weakGroup.size(); j++) 
                {
                    catident2[j - 1] = 0;
                }
    ...
    {
        clustalOutFile->close();
        cout << "A bad_alloc exception has occured in the clustalOut function.\n"
             << e.what() << "\n";
        exit(1);
    }
    catch(VectorOutOfRange e)
    {
        clustalOutFile->close();
        cout << "An exception has occured in the clustalOut function.\n"
             << e.what() << "\n";
        exit(1);    
    }
    catch(exception e)
                                ...
                                int len, int firstSeq)
{
    ...
    {
        cout << "An exception has occured in the findRangeValues function.\n"
             << e.what() << "\n";
        exit(1);    
    }    
    catch(...)
