...
int Tree::readTree(clustalw::Alignment* alignPtr, const string& treeFileName, int firstSeq, int lastSeq)
{
    ...
    if (numSeq >= 2)
    {
        ...
        for (i = firstSeq; i < lastSeq; i++)
        {
            ...
            {
                name1 = "";
                for (k = 0; k < lptr[j]->name.length() && k < clustalw::MAXNAMES; k++)
                {
                    c = lptr[j]->name[k];
                    if ((c > 0x40) && (c < 0x5b))
                    {
 ...
 * @return 
 */
void Tree::debugReportAllNodes(int nseqs)
{
    clustalw::TreeNode *p;
    int i,j ;
   float diff, maxDist;
    ...
    for (i = 0; i < ntotal; i++) {
        ...
        fprintf(stdout,
                ...
                i, p, p->parent, p->left, p->right, p->dist, diff);
    }
}
    ...
    for (i = 0; i < ntotal; i++)
    {
        ...
        if (p->parent == NULL)
        {
            /* AW Bug 94: p->parent must be chosen as rootNode
               ...
               needs serious dismantling anyway. See debugReportAllNodes
            */
